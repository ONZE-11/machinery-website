...
// section_key is the identifier — not mutable after creation
...
export default function EditHomepageSectionPage() {
  ...
  const onSubmit = async (data: EditableFields) => {
    ...
    if (!res.ok) {
      ...
      setError("root" as keyof EditableFields, { message: (json as { error?: string }).error ?? "Error al guardar" })
      return
    }
  ...
  if (notFound) return <div className="text-center py-24 text-muted-foreground">Section not found</div>
